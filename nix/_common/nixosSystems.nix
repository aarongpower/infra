{ inputs, nixpkgs ? inputs.nixpkgs, globals, root ? globals.flakeRoot, lib, self
, linuxModules, sharedModules, overlays, ... }:
let
  ...
  hosts = builtins.filter
    ...
    systemDirs;
in nixpkgs.lib.genAttrs hosts (host:
  let
    systemParams =
  ...
  in nixpkgs.lib.nixosSystem {
    ...
    modules = linuxModules ++ sharedModules ++ [
      ...
      } ++ lib.optional systemParams.useContainers ({ ... }:
        let
          generatedContainers =
